...
describe("Music Shop initial phase (e2e)", () => {
  ...
  it("registers a new client, returns a session and resolves it through auth/session", async () => {
    ...
    const createdCustomer = await dataSource
  ...
  it("clears the active session on logout", async () => {
    ...
    await agent.get("/api/v1/auth/session").expect(200).expect({
      session: null,
    });
  });
  ...
  it("returns backend-driven app config for locale and currency bootstrap", async () => {
    await request(app.getHttpServer())
      ...
      .expect((response) => {
        ...
        expect(response.body.appConfig.supportedLocales).toEqual([
          "ru",
          "en",
          "uz",
        ]);
  ...
  it("forbids clients from accessing business settings", async () => {
    ...
    await agent.get("/api/v1/settings").expect(403);
  });
  ...
  it("creates, updates, enriches media and deletes a product through admin flows", async () => {
    ...
    await agent
      ...
      .expect(201)
      .expect((response) => {
        expect(response.body.product.images).toContain(
          "/assets/custom/demo-pad.png",
        );
      });
    ...
    await agent
      ...
      .expect(201)
      .expect((response) => {
        expect(response.body.product.primaryImage).toBe(
          "/assets/custom/demo-pad.png",
        );
      });
    ...
    await request(app.getHttpServer())
      ...
      .expect((response) => {
        ...
        expect(response.body.authConfig.providers).not.toEqual(
          expect.arrayContaining([
            expect.objectContaining({ id: "staff-password" }),
          ]),
        );
  ...
  it("returns backend-driven navigation and permissions for admin-only sections", async () => {
    await request(app.getHttpServer())
      ...
      .expect(200)
      .expect((response) => {
        const routeIds = response.body.items.map(
    ...
    await request(app.getHttpServer())
      ...
      .expect(200)
      .expect((response) => {
        expect(Object.keys(response.body.permissions).sort()).toEqual([
          "admin",
          "client",
        ]);
  ...
  it("returns activity feed items with limit support for admin dashboards", async () => {
    ...
    await agent
      ...
      .expect((response) => {
        ...
        expect(response.body.items[0].messageKey).toBe(
  ...
  it("lists, updates and deletes employees through the admin API", async () => {
    ...
    await agent
      ...
      .expect((response) => {
        expect(
          response.body.some((item: { id: string }) => item.id === employeeId),
        ).toBe(true);
      });
  ...
  it("returns finance summary for admin users", async () => {
    ...
    await agent.get("/api/v1/finance/summary").expect(200).expect({
      revenue: 55500000,
  ...
  it("returns customer aggregates for admin customer list", async () => {
    ...
    await agent
      ...
      .expect((response) => {
        ...
        const amina = response.body.find(
          (item: { id: string }) => item.id === "customer-001",
        );
        expect(amina.fullName).toBe("Amina Karimova");
        expect(amina.ordersCount).toBe(1);
        expect(amina.repairsCount).toBe(1);
        expect(new Date(amina.registeredAt).toISOString()).toBe(
          amina.registeredAt,
        );
      });
  });
  ...
  it("returns category product counts for admin category list", async () => {
    ...
    await agent
      ...
      .expect(200)
      .expect((response) => {
        const pianos = response.body.find(
          (item: { id: string }) => item.id === "category-pianos",
        );
        const guitars = response.body.find(
          (item: { id: string }) => item.id === "category-guitars",
        );
    ...
    await agent
      ...
      .expect((response) => {
        const product = response.body.find(
          (item: { id: string }) => item.id === "product-player-strat",
        );
  ...
  it("returns repair admin fields and updates a repair", async () => {
    ...
    await agent
      ...
      .expect(200)
      .expect((response) => {
        const repair = response.body.items.find(
          (item: { id: string }) => item.id === "REP-2001",
        );
    ...
    await agent
      ...
      .expect((response) => {
        ...
        expect(response.body.repairRequest.receivedAt).toBe(
  ...
  it("creates public orders and public repairs without authentication", async () => {
    ...
    await request(app.getHttpServer())
      ...
      .expect((response) => {
        ...
        expect(response.body.repairRequest.instrumentName).toBe(
          "Digital piano",
        );
        expect(response.body.repairRequest.brand).toBe("Casio PX-S1100");
        expect(response.body.repairRequest.notes).toContain(
          "Public repair request",
        );
        expect(response.body.repairRequest.notes).toContain(
          "Photo: https://example.com/repair-photo.jpg",
        );
      });
  });
});
